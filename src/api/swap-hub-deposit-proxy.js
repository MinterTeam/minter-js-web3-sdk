...
import {HUB_DEPOSIT_PROXY_API_URL, HUB_DEPOSIT_PROXY_ETHEREUM_CONTRACT_ADDRESS, HUB_DEPOSIT_PROXY_BSC_CONTRACT_ADDRESS, NATIVE_COIN_ADDRESS, NETWORK_DATA} from '../config.js';
 ...
 * @return {Promise<{toTokenAmount: string, txList: Array<OneInchTx|EvmTxParams>}>}
 */
export async function buildSwapWithApproveTxList(chainId, swapParams, {idPreventConcurrency} = {}) {
    ...
    swapParams = {
        ...
        toTokenAddress: swapParams.toTokenAddress === NATIVE_COIN_ADDRESS ? NETWORK_DATA[chainId].wrappedNativeContractAddress : swapParams.toTokenAddress,
    };
...
export async function buildTxForSwapWithComposer(chainId, swapParams, {idPreventConcurrency} = {}) {
    const hubNetworkSlug = NETWORK_DATA[chainId].hubNetworkSlug;
